...
def _expand_part(part: str, total_pages: int) -> list:
    """展开单个页码部件（如 '3' 或 '5-7'）为 0-based 页码列表"""
    part = part.strip()
    if not part:
        return []
    if "-" in part:
...
def parse_page_range(range_str: str, total_pages: int) -> list:
    ...
    for part in range_str.split(","):
        ...
        if part == "ODD":
            ...
            pages.extend(_expand_part(part, total_pages))
...
def parse_page_range_ordered(range_str: str, total_pages: int) -> list:
    ...
    seen = set()
    for part in range_str.split(","):
        for i in _expand_part(part, total_pages):
            if i not in seen:
                pages.append(i)
                seen.add(i)
    return pages
...
class BaseTab:
    ...
    def resolve_target_pages(self, page_filter: str, total_pages: int) -> list:
        """解析页面过滤条件（支持 ODD/EVEN/范围）；空字符串时返回全部页面"""
        return (parse_page_range(page_filter, total_pages)
                if page_filter.strip() else list(range(total_pages)))

    def save_pdf(self, writer, out_path):
        """写入PDF并返回自动去重后的实际路径"""
...
class RotateTab(BaseTab):
    ...
    def _get_target_pages(self, total):
        ...
        return self.resolve_target_pages(self.rotate_pages_entry.get(), total)
    ...
    def rotate_pdf(self):
        ...
        try:
            ...
            pages = self._get_target_pages(total)
            if not pages:
...
class WatermarkTab(BaseTab):
    ...
    def add_watermark(self):
        ...
        try:
            ...
            reader = PdfReader(src)
            total = len(reader.pages)
            target_pages = self.resolve_target_pages(self.wm_pages.get(), total)
...
class CropTab(BaseTab):
    ...
    def crop_pdf(self):
        ...
        try:
            ...
            target_pages = self.resolve_target_pages(self.crop_pages.get(), total)
...
class ScaleTab(BaseTab):
    ...
    def scale_pdf(self):
        ...
        try:
            ...
            reader = PdfReader(src)
            total = len(reader.pages)
            target_pages = self.resolve_target_pages(self.scale_pages.get(), total)
...
class InfoTab(BaseTab):
    ...
    def _build(self):
        ...
        # 修复：滚动条应放在独立容器中与 Text 并排布局
        text_panel = ttk.Frame(frame)
        text_panel.pack(fill=tk.BOTH, expand=True, pady=4)

        self.info_text = tk.Text(text_panel, height=16, font=("Consolas", 9),
                                 wrap=tk.WORD, state=tk.DISABLED)
        scrollbar = ttk.Scrollbar(text_panel, orient=tk.VERTICAL,
...
class PdfToImageTab(BaseTab):
    ...
    def _browse_file(self, entry, key, auto_output_entry=None, auto_out_name=None):
        """模式感知的文件浏览：PDF→图片模式自动设置输出目录；图片→PDF模式直接添加图片"""
        ...
        if "PDF → 图片" in d:
            ...
            # ★ 自动设置输出目录为源文件所在目录
            self.out_dir_entry.delete(0, tk.END)
            self.out_dir_entry.insert(0, os.path.dirname(path))
        else:
            # 图片→PDF不需要单独选源文件，直接用图片列表
            self.add_images()

    def _on_mode_change(self, event=None):
        d = self.convert_mode.get()
        src_frame = getattr(self, "pi_frame", None)
